...
<form action="{{ route('parts.update', $part) }}" method="POST">
    ...
    <div class="mb-3">
        ...
        <select name="category_id" class="form-select" required>
            @foreach ($categories as $category)
                <option value="{{ $category->id }}" {{ $category->id == $part->category_id ? 'selected' : '' }}>{{ $category->name }}</option>
            @endforeach
        </select>
    </div>
    <div class="mb-3">
    ...
    <div class="mb-3">
        <label>Stok</label>
        <input type="number" name="stock_quantity" class="form-control" value="{{ $part->stock_quantity }}" required>
    </div>
    <div class="mb-3">
        <label>Minimal Stok</label>
        <input type="number" name="minimum_stock" class="form-control" value="{{ $part->minimum_stock }}" required>
    </div>
    <div class="mb-3">
        <label>Kondisi</label>
    ...
    <div class="form-check mb-3">
        <input class="form-check-input" type="checkbox" name="is_active" {{ $part->is_active ? 'checked' : '' }}>
