        ...
        <tbody>
            @foreach($compatibles as $compatible)
                <tr>
                    ...
                    <td>
                        <a href="{{ route('compatibles.edit', $compatible->id) }}" class="btn btn-sm btn-warning">Edit</a>
                        <form action="{{ route('compatibles.destroy', $compatible->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Yakin ingin menghapus kecocokan ini?')">
                            @csrf
                            @method('DELETE')
